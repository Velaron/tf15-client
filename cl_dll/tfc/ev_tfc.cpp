...
void EV_FireTFCShotgun( event_args_t *args )
{
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048.0, BULLET_PLAYER_TF_BUCKSHOT, 0, &tracerCount[idx - 1], 0.04, 0.04 );
}
...
void EV_FireTFCSuperShotgun(event_args_t *args)
{
	...
	EV_HLDM_FireBullets(idx, forward, right, up, 14, vecSrc, vecAiming, 2048.0, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx - 1], 0.04, 0.04);
}
...
void EV_TFC_Assault_WindUp(event_args_t *args)
{
	...
	idx = args->entindex;
	VectorCopy(args->origin, origin);
	g_flSpinDownTime[idx - 1] = 0.0;
	gEngfuncs.GetClientTime();
	g_flSpinUpTime[idx - 1] = 3.5;
	if (EV_IsLocal(idx))
